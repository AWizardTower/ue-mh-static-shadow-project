...
bool UMcpAutomationBridgeSubsystem::HandleControlEditorConsoleCommand(
  ...
  bool bSetConsoleVariableDirectly = false;
  FString CVarName;
  FString CVarValue;
  if (Command.Split(TEXT(" "), &CVarName, &CVarValue))
  {
    CVarName.TrimStartAndEndInline();
    CVarValue.TrimStartAndEndInline();

    if (!CVarName.IsEmpty() && !CVarValue.IsEmpty())
    {
      if (IConsoleVariable* ConsoleVariable = IConsoleManager::Get().FindConsoleVariable(*CVarName))
      {
        ConsoleVariable->Set(*CVarValue, ECVF_SetByConsole);
        bSetConsoleVariableDirectly = true;
      }
    }
  }
  ...
  if (GEngine)
  {
    bExecHandled = GEngine->Exec(World, *Command);
  }
  if (!bExecHandled)
  {
    bExecHandled = GEditor->Exec(World, *Command);
  }

  TSharedPtr<FJsonObject> Resp = McpHandlerUtils::CreateResultObject();
  Resp->SetBoolField(TEXT("success"), true);
  Resp->SetStringField(TEXT("command"), Command);
  Resp->SetBoolField(TEXT("setConsoleVariableDirectly"), bSetConsoleVariableDirectly);
  Resp->SetBoolField(TEXT("execHandled"), bExecHandled);
